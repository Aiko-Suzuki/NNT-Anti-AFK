...
function PANEL:Init()
    ...
    self.TextArea:SetTextColor(Color(255, 255, 255, 255))
...
derma.DefineControl("nnt-DNumSlider", "Menu Option Line", table.Copy(PANEL), "Panel")
...
function PANEL:Paint(w, h)
    ...
    function self.Slider:Paint(w, h)
        ...
        surface.DrawRect(8, h / 2 - 2, w - 15, 4)
    end

    function self.Slider.Knob:Paint(w, h)
        draw.RoundedBox(7, 0, 0, 16, 16, Color(76, 175, 80))
    end

    function self.Label:Paint()
...
function PANEL:SetActionFunction(func)
    self.OnValueChanged = function(self, val)
        func(self, "SliderMoved", val, 0)
    end
end

-- Compat
derma.DefineControl("nnt-slider", "", PANEL, "Panel")