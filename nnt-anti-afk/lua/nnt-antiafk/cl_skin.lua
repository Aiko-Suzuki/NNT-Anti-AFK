...
function SKIN:PaintPanel(panel, w, h)
    ...
    for k, v in pairs(panel.Title) do
        ...
        local a, _ = surface.GetTextSize(k)
...
function SKIN:PaintSliderKnob(panel, w, h)
    ...
    if (panel.Depressed) then
        nntdrawCircle(x, y, radius, seg, color2)
        nntdrawCircle(x, y, radius, seg, color)
    end
    ...
    nntdrawCircle(x, y, 11, seg, color3)
    nntdrawCircle(x, y, radius, seg, color)
    nntdrawCircle(x, y, 6, seg, color2)
    DisableClipping(false)
end
...
function SKIN:PaintComboBox(panel, w, h)
    ...
    if (panel:GetDisabled()) then return ComboBoxDisabled(0, 0, w, h) end
    if (panel.Depressed or panel:IsMenuOpen()) then return DrawSomething() end
    if (panel.Hovered) then return DrawSomething() end
