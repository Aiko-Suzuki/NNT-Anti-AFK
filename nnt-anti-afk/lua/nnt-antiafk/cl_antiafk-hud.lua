...
|  $$$$$$/|  $$$$$$$| $$  | $$|  $$$$$$$| $$     |  $$$$$$$| $$      |  $$$$$$/|  $$$$$$$  |  $$$$/|  $$$$/|  $$$$/| $$| $$  | $$|  $$$$$$$ /$$$$$$$/
 \______/  \_______/|__/  |__/ \_______/|__/      \_______/|__/       \______/  \_______/   \___/   \___/   \___/  |__/|__/  |__/ \____  $$|_______/ 
    ...
    NNTAddCheckBox("Freeze Salary (DarkRP)", 5, 1, "DARKPMONEY", GeneralSettingsM)
    NNTAddCheckBox("God Mode", 6, 1, "GODMODE", GeneralSettingsM)
    NNTAddCheckBox("Use Time Settings", 7, 1, "ENABLETIME", GeneralSettingsM)
    NNTAddCheckBox("Change Job/Team", 8, 1, "JOBENABLE", GeneralSettingsM)
    NNTAddCheckBox("Job/Team Revert Back", 9, 1, "JOBREVERT", GeneralSettingsM)
    --[[----------------------------------------------------------------------
        ...
        for k, v in pairs(NNTAntiafkThemes) do
            if k == value then
                ...
                if PreviewEnable then
                    if AfkPanelHUD ~= nil and AfkPanelHUD:IsValid() then
                        AfkPanelHUD:Close()
                    end
    ...
    net.Receive("nnt-antiak-settings", function()
        ...
        elseif data5 == "Settings" then
            -- to load separate data
            for k, v in pairs(data4) do
                if k == "WARN" then
                    WarnSliderSelect:SetValue(tonumber(v, 10) / 60)
                elseif k == "KICK" then
    ...
    net.Receive("AntiAfksenBypassUsers", function(len)
        ...
        for k, v in pairs(player.GetHumans()) do
            if not antiuserstring[v:SteamID()] then
